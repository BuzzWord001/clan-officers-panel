  ...
  const DICT = {
    ...
    "valor": {
      ...
      "Предупреждения": { fill: "mixed",
               what: "Все активные предупреждения человека в едином стиле «Предупреждение ⚠ N» (N — количество). ЦВЕТ показывает ТИП: 🟠 янтарный — норматив (авто), 🔴 красный — титул (офицер в игре), 🔵 синий — ручное (офицер на сайте). Тот же чип используется в колонках «Норматив» и «Титул».",
               how: "Наведи на чип — во всплывающей подсказке написано, что это за предупреждение, за что и как выдаётся, плюс детали (недели и % по нормативу, причины ручных). Норматив-предупреждение снимается при выполнении нормы (первым — самое суровое). Ручное добавляется кнопкой «+», снимается «✕». Титул снимается офицером в игре." },
