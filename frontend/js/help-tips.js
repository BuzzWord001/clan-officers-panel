...
(function () {
  ...
  const DICT = {
    ...
    "valor": {
      ...
      "Предупреждения": { fill: "calc",
               what: "Все активные предупреждения человека: за невыполнение норматива (с % набранного — цвет показывает строгость) и отметки офицера из титула.",
               how: "Каждая невыполненная неделя добавляет предупреждение (видно, сколько доблести он всё же набрал). Когда человек выполняет норматив, первым снимается самое СТРОГОЕ предупреждение, потом более мягкие. Отметка из титула снимается офицером в игре." },
