  ...
  const DICT = {
    ...
    "valor": {
      ...
      "Предупреждения": { fill: "mixed",
               what: "Все активные предупреждения человека: за невыполнение норматива (с % набранного — цвет показывает строгость), отметки офицера из титула, и ручные предупреждения офицера.",
               how: "Каждая невыполненная неделя добавляет предупреждение (видно, сколько доблести он всё же набрал). Когда человек выполняет норматив, первым снимается самое СТРОГОЕ предупреждение, потом более мягкие. Кнопкой «+» офицер может добавить предупреждение любой строгости вручную (снимается крестиком ✕)." },
