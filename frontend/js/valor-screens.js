...
(async function () {
  ...
  // ── «Готово»: перечитать неделю, подтвердить актуальность И заявить публикацию
  //    топа (через 5 минут пост во все соцсети сделает локальный поллер). ──
  async function doneRefresh() {
    if (!openWeek) { toast("Сначала выбери неделю"); return; }
    await loadWeek(openWeek, activeFolder());
    try {
      await API.valorRequestPublish();
      toast("Готово — данные обновлены. Пост с ТОПом во все соцсети выйдет через ~5 минут.");
    } catch (e) {
      toast("Данные обновлены, но заявку на пост не удалось создать: " + (e.detail || e.message));
    }
  }
  ...
  function weekOverlap(frame) {
    ...
    // 3) Перекрытие из ДАННЫХ (autoOverlap = строк 1-го кадра − новых во 2-м) —
    //    само подстраивается под шаг прокрутки ЛЮБОЙ недели. Раньше приоритет был
    //    у сохранённого d.off, который задавался под старый шаг (5 скроллов) и при
    //    смене на 8 смещал строки. Теперь d.off — только запас, если данных нет.
    const ao = autoOverlap();
    if (ao > 0) return ao;
    return (d && d.off != null) ? d.off : ao;
  }
