...
(async function () {
  ...
  const TAG_META = {
    ...
    elite:      { label: "Элита", icon: "⚔", color: "#ff5a6a",
                  cls: "tag-elite",
                  tip: "Топ по урону. Роль присваивает офицер вручную." },
  ...
  const ROLE_HOWTO = {
    ...
    elite:      "Элита клана — Топ по урону. Роль присваивает офицер вручную (в реестре или в таблице).",
  ...
  function injectEditStyles() {
    ...
    s.textContent = `
      ...
      .radm.has-spouse{opacity:1;border-color:#e08ac0;color:#ffcbe8;background:rgba(224,138,192,.12)}
  ...
  $("valor-tbody").addEventListener("click", async (ev) => {
    ...
    // «В архив» и «супруг» — офицеру и админу; остальные правки — только админ.
    if (act === "archive" || act === "spouse") { if (!IS_OFFICER) return; }
  ...
  $("valor-tbody").addEventListener("click", async (ev) => {
    ...
    if (addBtn) {
      ...
      const tag = (prompt(
        `Добавить роль для «${nick}»?\nНапример: veteran (Ветеран), elite (Элита — Топ по урону)`,
        "elite") || "").trim();
      if (!tag) return;
      try {
