...
print(f"{'ник':22} {'нед':>3} {'с_недели':>9} {'за_посл':>7} {'всего':>6}")
print("-" * 56)
for m in sorted(afk, key=lambda x: -((x.get('afk_info') or {}).get('weeks') or 0)):
    ...
    print(f"{nick:22} {str(a.get('weeks','?')):>3} "
          ...
          f"{str(a.get('valor_last','—')):>7} "
          f"{str(a.get('valor_total','—')):>6}")
    if a.get("weekly"):
        wk = ", ".join(f"{w['week']}:{w['valor']}" for w in a["weekly"])
        print(f"{'':22}   по неделям: {wk}")

# 3) Те, кто набирал доблесть ДАЖЕ в АФК (суммарно > 0) — главный кейс.
earning = [m for m in afk if (m.get("afk_info") or {}).get("valor_total", 0)
           and (m["afk_info"]["valor_total"] > 0)]
print(f"\nНабирали доблесть во время АФК: {len(earning)}")
for m in earning:
    a = m["afk_info"]
    print(f"  {m.get('nick')}: всего {a['valor_total']} за {a['weeks']} нед.")
