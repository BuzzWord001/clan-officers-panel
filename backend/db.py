...
# Аддитивные ветки (без множителя доблести). Иерархия: ветеран > офицер >
# общительность; все вместе ≪ доблести (она доминирует за счёт базы×множитель).
VALOR_W_VETERAN = 11  # руна ветерана (флэт; потолок 11)
VALOR_W_OFFICER = 7   # база офицерства × множитель ≤~×1.4 → потолок ~9.8
VALOR_W_VK   = 2      # руна ВКонтакте
VALOR_W_TG   = 2      # руна Telegram
VALOR_W_CHAT = 3      # руна общительности (актив из «Участники»); ветка ≤8.4
...
_MAG_BASE = {"over": 12.0, "double": 18.0, "triple": 26.0, "record": 34.0,
             "phenom": 42.0, "titan": 48.0, "overlord": 54.0, "absolute": 60.0}
MAG_BASE_MAX = 60.0
