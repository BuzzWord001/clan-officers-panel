    ...
    with db.connection() as conn:
        conn.execute(
            """INSERT INTO auth_config
               (id, officer_password_hash, admin_username, admin_password_hash, updated_at)
               VALUES (1, ?, ?, ?, ?)""",
            (
...
def set_officer_password(new_password: str) -> None:
    ...
    with db.connection() as conn:
        conn.execute(
            "UPDATE auth_config SET officer_password_hash = ?, updated_at = ? WHERE id = 1",
            (_hash(new_password), now),
        )
