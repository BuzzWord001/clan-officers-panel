...
def _valor_week_stale() -> str:
    ...
    if week == now_week:
        return ""
    # РАБОЧИЙ ЦИКЛ КЛАНА: доблесть собирают в воскресенье вечером, а раздают и двигают очередь
    # уже ночью или в понедельник — то есть на СЛЕДУЮЩЕЙ ISO-неделе. Это норма, а не «старая
    # доблесть», и ругаться тут нельзя: 17.08.2026 в 00:10 МСК защита заблокировала Лиру сдвиг
    # по свежесобранной вчера доблести. Терпим ровно один шаг назад и только в начале недели
    # (пн/вт); дальше — снимок действительно устарел.
    msk = datetime.now(timezone(timedelta(hours=3)))
    prev = msk - timedelta(days=7)
    py, pw, _pd = prev.isocalendar()
    if week == "%d-W%02d" % (py, pw) and msk.isoweekday() <= 2:
        return ""
