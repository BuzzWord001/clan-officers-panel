...
print("\n10. ДОБЛЕСТЬ СЧИТАЕТСЯ ПО ЧЕЛОВЕКУ, А НЕ ПО ОТДЕЛЬНОМУ ПЕРСОНАЖУ")
# 16.08.2026: Риcлинг стоял в ТОП-20 пятым со 110 (доблесть набрал его твин) и одновременно
# был «мало доблести» в очереди за цилинём с порогом 100 — потому что порог проверялся по
# мэйну (76). Человек один: и в топе, и на пороге очереди берётся лучший из его персонажей.
MAIN = {"твин": "мэйн", "мэйн": "мэйн"}
rep = run({2: [entry("Мэйн", "drakonya-cheshuya", 1, 2, main_canon="мэйн", canon_nick="мэйн")]},
          {"мэйн": 76, "твин": 110}, main_map=MAIN)
rows = {r["nick"]: r for Q in rep["queues"] for r in Q["rows"]}
check(rows["Мэйн"]["valor"] == 110, "порог считает лучший результат мэйна и твинов (110, не 76)")
check("drakonya-cheshuya" in (rows["Мэйн"]["got"] or {}),
      "человек с твином выше порога получает легендарный ресурс")
# и обратно: без твинов ничего не меняется — берётся собственная доблесть
rep2 = run({2: [entry("Один", "drakonya-cheshuya", 1, 2, main_canon="один", canon_nick="один")]},
           {"один": 76}, main_map={"один": "один"})
rows2 = {r["nick"]: r for Q in rep2["queues"] for r in Q["rows"]}
check(rows2["Один"]["valor"] == 76 and not (rows2["Один"]["got"] or {}),
      "у кого твинов нет — порог по своей доблести, ниже порога не выдаём")
# цилинь-очередь подчиняется тому же правилу
rep3 = run({2: [entry("Мэйн", "mount-cilin", 1, 2, main_canon="мэйн", canon_nick="мэйн")]},
           {"мэйн": 76, "твин": 110}, main_map=MAIN)
check([p["status"] for p in rep3["pet_queue"]] == ["pet"],
      "в очереди цилиня тоже: человек с твином выше порога — годен")
