...
def valor_request_publish_ep(actor: dict = Depends(require_officer)) -> dict:
    ...
    жетонов ТОП-3 по доблести. Локальный поллер (тик раз в минуту, без доп. паузы)
    опубликует пост во все соцсети. Жетоны ТОП-3 начисляются здесь ИДЕМПОТЕНТНО (за неделю — один раз), чтобы
