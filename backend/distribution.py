...
Пороги доблести: обычные (оч.0) ≥60, редкие/легендарные (оч.1/2) ≥100, мифические (оч.3) ≥200.
...
QUEUE_THRESHOLD = {0: 60, 1: 100, 2: 100, 3: 200}   # мифические (SS) с 2026-W30 → 200
...
def compute(state: dict, valor_map: dict, cfg: dict) -> dict:
    ...
    for q in (0, 1, 2, 3):
        ...
        # ПОРОГ ДОБЛЕСТИ ДЕЙСТВУЕТ ВО ВСЕХ ОЧЕРЕДЯХ, включая легендарную q2.
        # История: 02.08 порог для q2 сняли («сколько выпало — столько и раздаём сверху
        # очереди, даже если у последнего доблесть чуть ниже»). 09.08 это вылезло боком:
        # Драконью чешую получил EvgeniY с 0 доблести, а Strannik и Мortаlitу (по 107) —
        # нет, потому что стоят ниже. Лир вернул общее правило: не набрал порог — не получаешь,
        # ресурс уходит следующему, кто набрал.
